...
- Gate4 增补：
  ...
  - Step 60：把 Step59 compare_bundle.json 纳入 run-end evidence gate / CI gate（与 Step54 compare 同目录，生成后必须 verify，并纳入 FILELIST/SHA256SUMS/evidence_ref_index；工具/证据失败=exit 2），让研究消费入口也进入可复核闭环（规格见：`docs/v11/V11_STEP60_GATE_COMPARE_BUNDLE_INTEGRATION_20251230.md`）。
