...
- Gate4 增补：
  ...
  - Step 72.1：代谢候选指标与筛选门槛（必要且可行）：冻结最小集合（决策耗时 + API 占用），明确暂缓项与观察污染控制（规格见：`docs/v11/V11_STEP72_1_AGENT_METABOLISM_CANDIDATES_AND_FILTER_20251230.md`）。
  - Step 72.2：代谢实施前 checklist + “寿命三视角”去围栏化定义（L_time/L_exec/L_budget），并给出“高 ROI 可能被高代谢吃掉”的 fact-only 表述边界（规格见：`docs/v11/V11_STEP72_2_AGENT_METABOLISM_PREIMPLEMENTATION_CHECKLIST_AND_LIFESPAN_VIEWS_20251230.md`）。
  - Step 72（Quant 落地记录）：`docs/v11/V11_STEP72_AGENT_METABOLISM_OBSERVABILITY_CONTRACT_IMPLEMENTED_IN_QUANT_20251230.md`（commit：3881893）。
  - Step 73：CI 必跑 Step72 代谢产物验收：确保 agent_metabolism_summary.json 存在、schema 满足、entry.json 索引存在且与 evidence_ref_index 一致；禁止跳过，失败阻断合并（规格见：`docs/v11/V11_STEP73_CI_GATE_STEP72_METABOLISM_ACCEPTANCE_20251230.md`）。
  - Step 73（Quant 落地记录）：`docs/v11/V11_STEP73_CI_GATE_STEP72_METABOLISM_ACCEPTANCE_IMPLEMENTED_IN_QUANT_20251230.md`（commit：e51390d）。
  - Step 74：实时对账冻结（execution_frozen）：证据链/真值链断裂时立即冻结，禁止继续下单污染证据链，并写实落盘 errors/manifest（规格见：`docs/v11/V11_STEP74_REAL_TIME_RECONCILIATION_FREEZE_20251230.md`）。
  - Step 74（Quant 落地记录）：`docs/v11/V11_STEP74_REAL_TIME_RECONCILIATION_FREEZE_IMPLEMENTED_IN_QUANT_20251230.md`（commit：121095a）。
  - Step 75：CI 必跑 Step74 冻结机制验收（`tools/test_step74_execution_freeze.py`），禁止跳过，失败阻断合并（规格见：`docs/v11/V11_STEP75_CI_GATE_STEP74_EXECUTION_FREEZE_ACCEPTANCE_20251230.md`）。
  ...
  - Step 84（Quant 落地记录）：`docs/v11/V11_STEP84_EVIDENCE_REFS_MULTI_LINE_RANGE_VALIDATION_IMPLEMENTED_IN_QUANT_20251230.md`（commits：7f352f1 + 9084fa9）。
...
## 5) First Flight / Real Flight Rollout Protocol（truth-first, additive-only）

为减少“反复跑但无法确认真值/入口混用”的节奏损耗，V11 baseline 追加冻结以下四阶段推进顺序（只追加，不替换旧 Step89 叙述）：

- Stage 1 — **Mac First Flight**：使用专用 `first_flight` 入口在 Mac 本地逐模块做 truth-backed 测试（必须真实对接交易所 demo 并落盘真值：至少 `orders_history.jsonl`；若 filled 则 `fills.jsonl`/`bills.jsonl` 必须可 join）。
- Stage 2 — **VPS First Flight**：在 VPS 复现同一套 First Flight（同一 Quant main HEAD），验证容器/权限/限流/时钟条件下真值链仍可落盘。
- Stage 3 — **Mac Real Flight**：实现并验证 `real_flight` 干净入口（仅负责真实运行与落盘，不编排 tools）。
- Stage 4 — **VPS Real Flight**：部署 `real_flight` 到 VPS，纳入 Step93 稳定窗口运行纪律与周检/日检。
