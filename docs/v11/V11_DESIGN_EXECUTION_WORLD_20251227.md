...
关系声明（重要）：
- **SSOT（变更口径）**：`docs/v11/V11_BASELINE_CHANGELOG_20251226.md`（只增不改）
...
相关附录：
- **V11 execution_world 运行闭包白名单（closure allowlist）**：`docs/v11/V11_EXECUTION_WORLD_CLOSURE_ALLOWLIST_20251227.md`
...
参考：
- `docs/v11/V11_BASELINE_CHANGELOG_20251226.md`（§2.1）
...
- `docs/v11/V11_STEP29_CI_EVIDENCE_GATE_STEP26_20251229.md`
Step 30（Quant runner 已强制接入 run-end evidence gate：不可绕过，FAIL/ERROR=exit 2）：
- `docs/v11/V11_STEP30_RUN_END_EVIDENCE_GATE_IMPLEMENTED_20251229.md`
Step 31（扩展 verifier 到 Tier-1：Ledger + ProbeGating + Errors 的最小一致性）：
- `docs/v11/V11_STEP31_EXTEND_EVIDENCE_VERIFIER_TIER1_20251229.md`
Step 31（Quant 落地记录：CI gate 与 run-end gate 自动升级覆盖 Tier-1）：
- `docs/v11/V11_STEP31_TIER1_VERIFIER_IMPLEMENTED_IN_QUANT_20251229.md`
...
Step 32（Quant 落地记录）：
- `docs/v11/V11_STEP32_ORPHAN_DETECTION_IMPLEMENTED_IN_QUANT_20251229.md`
Step 33（P3/P4 最小可测：fills/bills 分页闭合 + join + 幂等）：
- `docs/v11/V11_STEP33_FILLS_BILLS_JOIN_MIN_MEASURABLE_20251229.md`
Step 33（Quant 落地记录）：
- `docs/v11/V11_STEP33_FILLS_BILLS_JOIN_IMPLEMENTED_IN_QUANT_20251229.md`
Step 34（分页闭合证据：paging_traces.jsonl append-only，作为 closure proof）：
- `docs/v11/V11_STEP34_PAGING_TRACES_APPEND_ONLY_20251229.md`
Step 34（Quant 落地记录）：
- `docs/v11/V11_STEP34_PAGING_TRACES_IMPLEMENTED_IN_QUANT_20251229.md`
Step 35（gates 强制要求 paging_traces：verifier/run-end/CI，不可绕过）：
- `docs/v11/V11_STEP35_REQUIRE_PAGING_TRACES_IN_GATES_20251229.md`
Step 35（Quant 落地记录）：
- `docs/v11/V11_STEP35_REQUIRE_PAGING_TRACES_IMPLEMENTED_IN_QUANT_20251229.md`
Step 36（冻结 auditor_report 的 paging coverage 字段：page_count/closure_proved/trace_range）：
- `docs/v11/V11_STEP36_AUDITOR_PAGING_COVERAGE_FIELDS_20251229.md`
Step 36（Quant 落地记录）：
- `docs/v11/V11_STEP36_AUDITOR_PAGING_COVERAGE_IMPLEMENTED_IN_QUANT_20251229.md`
Step 37（paging query_chain_id/scope_id：防 paging_traces 混链歧义）：
- `docs/v11/V11_STEP37_PAGING_QUERY_CHAIN_ID_20251229.md`
Step 37（Quant 落地记录）：
- `docs/v11/V11_STEP37_PAGING_QUERY_CHAIN_ID_IMPLEMENTED_IN_QUANT_20251229.md`
Step 38（run_manifest 冻结 audit scope anchors，防审计范围漂移）：
- `docs/v11/V11_STEP38_AUDIT_SCOPE_ANCHORS_IN_MANIFEST_20251229.md`
Step 38（Quant 落地记录）：
- `docs/v11/V11_STEP38_AUDIT_SCOPE_ANCHORS_IMPLEMENTED_IN_QUANT_20251229.md`
Step 39（audit_scope_id 作为全局主锚点：跨文件 join 与 verifier fail-closed）：
- `docs/v11/V11_STEP39_AUDIT_SCOPE_ID_AS_GLOBAL_ANCHOR_20251229.md`
Step 39（Quant 落地记录）：
- `docs/v11/V11_STEP39_AUDIT_SCOPE_ID_GLOBAL_ANCHOR_IMPLEMENTED_IN_QUANT_20251229.md`
Step 40（run_manifest.audit_scopes[]：多次审计 append-only，不覆盖）：
- `docs/v11/V11_STEP40_MANIFEST_AUDIT_SCOPES_APPEND_ONLY_20251229.md`
Step 40（Quant 落地记录）：
- `docs/v11/V11_STEP40_MANIFEST_AUDIT_SCOPES_IMPLEMENTED_IN_QUANT_20251229.md`
Step 41（审计链 evidence_refs 统一协议：可机器追踪 + SHA256 覆盖）：
- `docs/v11/V11_STEP41_EVIDENCE_REFS_STANDARD_20251229.md`
Step 41（Quant 落地记录）：
- `docs/v11/V11_STEP41_EVIDENCE_REFS_STANDARD_IMPLEMENTED_IN_QUANT_20251229.md`
Step 42（审计链 evidence_refs 强约束升级：gate-on 时 sha256_16 强制，jsonl 行号强制）：
- `docs/v11/V11_STEP42_EVIDENCE_REFS_HARDENING_20251229.md`
Step 42（Quant 落地记录）：
- `docs/v11/V11_STEP42_EVIDENCE_REFS_HARDENING_IMPLEMENTED_IN_QUANT_20251229.md`
Step 43（evidence_ref_index：把 hash/行号复算成本从 verifier 挪到 run-end gate）：
- `docs/v11/V11_STEP43_EVIDENCE_REF_INDEX_20251229.md`
Step 43（Quant 落地记录）：
- `docs/v11/V11_STEP43_EVIDENCE_REF_INDEX_IMPLEMENTED_IN_QUANT_20251229.md`
Step 44（evidence_refs 可解引用校验：防止“形式正确但指向无关行”）：
- `docs/v11/V11_STEP44_EVIDENCE_REFS_DEREFERENCE_VALIDATION_20251229.md`
Step 44（Quant 落地记录）：
- `docs/v11/V11_STEP44_EVIDENCE_REFS_DEREFERENCE_VALIDATION_IMPLEMENTED_IN_QUANT_20251229.md`
Step 45（evidence_refs 语义 join 校验：防止同 run 内“指向无关行”）：
- `docs/v11/V11_STEP45_EVIDENCE_REFS_SEMANTIC_JOIN_VALIDATION_20251229.md`
Step 45（Quant 落地记录）：
- `docs/v11/V11_STEP45_EVIDENCE_REFS_SEMANTIC_JOIN_VALIDATION_IMPLEMENTED_IN_QUANT_20251229.md`
Step 46（Evidence Gate Bundle 冻结：contract_version/fixtures/exit codes/artifacts）：
- `docs/v11/V11_STEP46_EVIDENCE_GATE_BUNDLE_FREEZE_20251229.md`
Step 46（Quant 落地记录）：
- `docs/v11/V11_STEP46_EVIDENCE_GATE_BUNDLE_FREEZE_IMPLEMENTED_IN_QUANT_20251229.md`
Step 47（基因/特征/账簿 SSOT 总审计：has_position 退役，I/C 口径冻结）：
- `docs/v11/V11_STEP47_GENE_FEATURE_LEDGER_SSOT_20251230.md`
Step 47（Quant 落地记录）：
- `docs/v11/V11_STEP47_GENE_FEATURE_LEDGER_SSOT_IMPLEMENTED_IN_QUANT_20251230.md`
Step 48（I/C 注入 SSOT：Ledger triad → I probes；C_prev_net_intent(t-1)）：
- `docs/v11/V11_STEP48_I_C_PROBES_INJECTION_SSOT_20251230.md`
Step 48（Quant 落地记录）：
- `docs/v11/V11_STEP48_I_C_PROBES_INJECTION_SSOT_IMPLEMENTED_IN_QUANT_20251230.md`
Step 49（C 消融开关：C_off vs C_on，manifest/contract 证据化）：
- `docs/v11/V11_STEP49_C_ABLATION_SWITCH_20251230.md`
Step 49（Quant 落地记录）：
- `docs/v11/V11_STEP49_C_ABLATION_SWITCH_IMPLEMENTED_IN_QUANT_20251230.md`
Step 50（C_off vs C_on 最小对照实验模板：manifest.ablation_experiment 冻结）：
- `docs/v11/V11_STEP50_MIN_ABLATION_EXPERIMENT_TEMPLATE_20251230.md`
Step 50（Quant 落地记录）：
- `docs/v11/V11_STEP50_MIN_ABLATION_EXPERIMENT_TEMPLATE_IMPLEMENTED_IN_QUANT_20251230.md`
Step 51（最小对照实验跑通 + ablation_summary.json 事实统计产物）：
- `docs/v11/V11_STEP51_MIN_ABLATION_RUN_AND_SUMMARY_20251230.md`
Step 51（Quant 落地记录）：
- `docs/v11/V11_STEP51_MIN_ABLATION_RUN_AND_SUMMARY_IMPLEMENTED_IN_QUANT_20251230.md`
Step 52（Feature dim ↔ DecisionEngine ↔ Genome schema 一致性冻结，fail-closed）：
- `docs/v11/V11_STEP52_FEATURE_DIM_DECISION_ENGINE_GENOME_ALIGNMENT_20251230.md`
Step 52（Quant 落地记录）：
...
Step 54（把 Step 53 compare 纳入 run-end/CI gates，使 compare 进入证据包闭环）：
- `docs/v11/V11_STEP54_STEP53_GATE_INTEGRATION_20251230.md`
- Step 54（Quant 落地记录）：
  - `docs/v11/V11_STEP54_STEP53_GATE_INTEGRATION_IMPLEMENTED_IN_QUANT_20251230.md`

Step 56（CI 必跑 gate：要求 Step55 验收脚本在 Quant CI 中必跑，阻断回归）：
...
Step 65（research_bundle：研究侧消费入口目录布局与归档边界）：
- `docs/v11/V11_STEP65_RESEARCH_BUNDLE_OUTPUT_CONTRACT_20251230.md`
- Step 65（Quant 落地记录）：
  - `docs/v11/V11_STEP65_RESEARCH_BUNDLE_OUTPUT_CONTRACT_IMPLEMENTED_IN_QUANT_20251230.md`
...
Step 67（research_bundle 单入口 entry.json：研究侧一键加载/校验入口）：
- `docs/v11/V11_STEP67_RESEARCH_BUNDLE_ENTRYPOINT_CONTRACT_20251230.md`
...
- Step 74（Quant 落地记录）：
  - `docs/v11/V11_STEP74_REAL_TIME_RECONCILIATION_FREEZE_IMPLEMENTED_IN_QUANT_20251230.md`

Step 75（CI gate：必跑 Step74 集成测试，锁定 freeze 语义）：
- `docs/v11/V11_STEP75_CI_GATE_STEP74_EXECUTION_FREEZE_ACCEPTANCE_20251230.md`
- Step 75（Quant 落地记录）：
  - `docs/v11/V11_STEP75_CI_GATE_STEP74_EXECUTION_FREEZE_ACCEPTANCE_IMPLEMENTED_IN_QUANT_20251230.md`

Step 76（freeze E2E：冻结 pre-check 强制接入所有写路径，证明冻结后不触达 connector）：
...
Step 89（real run acceptance：Mac preflight → VPS container 最小闭环验收）：
- `docs/v11/V11_STEP89_REAL_RUN_ACCEPTANCE_MAC_PREFLIGHT_VPS_CONTAINER_20251230.md`
