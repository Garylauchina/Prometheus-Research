...
> **V11 文档入口（新写入位置）**：从现在开始，V11 新增文档只写入 `docs/v11/`（入口：`docs/v11/V11_RESEARCH_INDEX.md`）。  
> 当前 `docs/v10/` 内的 V11 文档为历史遗留，后续将渐进迁移并保留 stub 指针文件，避免旧链接断裂。
...
- **V11 Step 27（Step26 证据包最小复核清单 + 一键校验脚本）**：
  - Checklist：`docs/v11/V11_STEP27_STEP26_EVIDENCE_MIN_REVIEW_CHECKLIST_20251229.md`
  - Verifier：`tools/verify_step26_evidence.py`
- **V11 Step 28（run-end 证据包打包 + Gate，fail-closed）**：
  - Spec：`docs/v11/V11_STEP28_EVIDENCE_PACKAGING_GATE_20251229.md`
- **V11 Step 29（CI Evidence Gate：fixture-based，fail-closed）**：
  - Spec：`docs/v11/V11_STEP29_CI_EVIDENCE_GATE_STEP26_20251229.md`
- **V11 Step 30（Quant runner 强制 run-end evidence gate，fail-closed）**：
  - Record：`docs/v11/V11_STEP30_RUN_END_EVIDENCE_GATE_IMPLEMENTED_20251229.md`
- **V11 Step 31（扩展 verifier 到 Tier-1：Ledger + ProbeGating + Errors 一致性）**：
  - Spec：`docs/v11/V11_STEP31_EXTEND_EVIDENCE_VERIFIER_TIER1_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP31_TIER1_VERIFIER_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 32（Orphan Detection 最小可测：orders-level）**：
  - Spec：`docs/v11/V11_STEP32_ORPHAN_DETECTION_MIN_MEASURABLE_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP32_ORPHAN_DETECTION_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 33（fills/bills join 最小可测：P3/P4）**：
  - Spec：`docs/v11/V11_STEP33_FILLS_BILLS_JOIN_MIN_MEASURABLE_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP33_FILLS_BILLS_JOIN_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 34（分页闭合证据落盘：paging_traces.jsonl）**：
  - Spec：`docs/v11/V11_STEP34_PAGING_TRACES_APPEND_ONLY_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP34_PAGING_TRACES_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 35（gates 强制要求 paging_traces，不可绕过）**：
  - Spec：`docs/v11/V11_STEP35_REQUIRE_PAGING_TRACES_IN_GATES_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP35_REQUIRE_PAGING_TRACES_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 36（冻结 auditor_report 的 paging coverage 字段）**：
  - Spec：`docs/v11/V11_STEP36_AUDITOR_PAGING_COVERAGE_FIELDS_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP36_AUDITOR_PAGING_COVERAGE_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 37（paging query_chain_id/scope_id：防混链）**：
  - Spec：`docs/v11/V11_STEP37_PAGING_QUERY_CHAIN_ID_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP37_PAGING_QUERY_CHAIN_ID_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 38（run_manifest 冻结 audit scope anchors）**：
  - Spec：`docs/v11/V11_STEP38_AUDIT_SCOPE_ANCHORS_IN_MANIFEST_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP38_AUDIT_SCOPE_ANCHORS_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 39（audit_scope_id 全局主锚点：跨文件 join）**：
  - Spec：`docs/v11/V11_STEP39_AUDIT_SCOPE_ID_AS_GLOBAL_ANCHOR_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP39_AUDIT_SCOPE_ID_GLOBAL_ANCHOR_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 40（run_manifest.audit_scopes[]：append-only 多次审计记录）**：
  - Spec：`docs/v11/V11_STEP40_MANIFEST_AUDIT_SCOPES_APPEND_ONLY_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP40_MANIFEST_AUDIT_SCOPES_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 41（审计链 evidence_refs 统一协议）**：
  - Spec：`docs/v11/V11_STEP41_EVIDENCE_REFS_STANDARD_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP41_EVIDENCE_REFS_STANDARD_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 42（审计链 evidence_refs 强约束升级）**：
  - Spec：`docs/v11/V11_STEP42_EVIDENCE_REFS_HARDENING_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP42_EVIDENCE_REFS_HARDENING_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 43（evidence_ref_index：hash+line_count 索引）**：
  - Spec：`docs/v11/V11_STEP43_EVIDENCE_REF_INDEX_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP43_EVIDENCE_REF_INDEX_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 44（evidence_refs 可解引用校验）**：
  - Spec：`docs/v11/V11_STEP44_EVIDENCE_REFS_DEREFERENCE_VALIDATION_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP44_EVIDENCE_REFS_DEREFERENCE_VALIDATION_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 45（evidence_refs 语义 join 校验）**：
  - Spec：`docs/v11/V11_STEP45_EVIDENCE_REFS_SEMANTIC_JOIN_VALIDATION_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP45_EVIDENCE_REFS_SEMANTIC_JOIN_VALIDATION_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 46（Evidence Gate Bundle 冻结）**：
  - Spec：`docs/v11/V11_STEP46_EVIDENCE_GATE_BUNDLE_FREEZE_20251229.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP46_EVIDENCE_GATE_BUNDLE_FREEZE_IMPLEMENTED_IN_QUANT_20251229.md`
- **V11 Step 47（基因/特征/账簿 SSOT 总审计）**：
  - Spec：`docs/v11/V11_STEP47_GENE_FEATURE_LEDGER_SSOT_20251230.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP47_GENE_FEATURE_LEDGER_SSOT_IMPLEMENTED_IN_QUANT_20251230.md`
- **V11 Step 48（I/C 注入 SSOT）**：
  - Spec：`docs/v11/V11_STEP48_I_C_PROBES_INJECTION_SSOT_20251230.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP48_I_C_PROBES_INJECTION_SSOT_IMPLEMENTED_IN_QUANT_20251230.md`
- **V11 Step 49（C 消融开关）**：
  - Spec：`docs/v11/V11_STEP49_C_ABLATION_SWITCH_20251230.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP49_C_ABLATION_SWITCH_IMPLEMENTED_IN_QUANT_20251230.md`
- **V11 Step 50（最小对照实验模板：C_off vs C_on）**：
  - Spec：`docs/v11/V11_STEP50_MIN_ABLATION_EXPERIMENT_TEMPLATE_20251230.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP50_MIN_ABLATION_EXPERIMENT_TEMPLATE_IMPLEMENTED_IN_QUANT_20251230.md`
- **V11 Step 51（最小对照实验跑通 + ablation_summary.json）**：
  - Spec：`docs/v11/V11_STEP51_MIN_ABLATION_RUN_AND_SUMMARY_20251230.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP51_MIN_ABLATION_RUN_AND_SUMMARY_IMPLEMENTED_IN_QUANT_20251230.md`
- **V11 Step 52（Feature dim ↔ DecisionEngine ↔ Genome schema 一致性冻结）**：
  - Spec：`docs/v11/V11_STEP52_FEATURE_DIM_DECISION_ENGINE_GENOME_ALIGNMENT_20251230.md`
  - Record（Quant 落地）：`docs/v11/V11_STEP52_FEATURE_DIM_DECISION_ENGINE_GENOME_ALIGNMENT_IMPLEMENTED_IN_QUANT_20251230.md`
- **V11 Step 53（ablation_compare.json 对照聚合报告）**：
  - Spec：`docs/v11/V11_STEP53_ABLATION_COMPARE_REPORT_20251230.md`
