...
from .config_multi_market import CONFIG_MULTI_MARKET, generate_multi_market_gene, mutate_multi_market_gene
import pandas as pd
from typing import Dict, List

class PrometheusV3MultiMarket:
    """Prometheus v3.0 多市场交易系统（简化版）"""
    ...
    def run(self, data: pd.DataFrame) -> Dict:
        ...
        for day in range(len(data)):
            ...
            # 检查死亡
            self._check_deaths(day)
            
            # 检查繁殖
            self._check_reproduction(day)
    ...
    def _check_deaths(self, day):
        """检查死亡"""
        death_config = self.config['agent_manager']['death']
        
        if not death_config['enabled']:
            return
        ...
        for agent in self.agents:
            ...
            # ROI过低死亡
            if agent.roi < death_config['roi_threshold']:
                agent.is_alive = False
                agent.death_reason = 'low_roi'
                self.stats['total_deaths'] += 1
            
            # 长期无交易死亡
            elif day - agent.last_trade_day > death_config['max_inactive_days']:
                agent.is_alive = False
                agent.death_reason = 'inactive'
                self.stats['total_deaths'] += 1
    
    def _check_reproduction(self, day):
        """检查繁殖"""
        repro_config = self.config['agent_manager']['reproduction']
        
        if not repro_config['enabled']:
            return
        
        # 检查是否达到最大数量
        active_count = len([a for a in self.agents if a.is_alive])
        if active_count >= self.config['agent_manager']['max_agents']:
            return
        ...
        for agent in self.agents:
            ...
            if (agent.roi >= repro_config['min_roi'] and 
                agent.trade_count >= repro_config['min_trades']):
                ...
                if total_capital > 0:
                    # 变异基因
                    new_gene = mutate_multi_market_gene(agent.gene, repro_config['mutation_rate'])
    ...
    def get_system_roi(self) -> float:
        """计算系统总ROI"""
        total_capital = sum(a.capital for a in self.agents if a.is_alive)
    ...
    def _collect_results(self) -> Dict:
        ...
        # 计算总交易和费用
        total_spot_trades = sum(a.spot_account.trades for a in self.agents)
        total_futures_trades = sum(a.futures_account.trades for a in self.agents)
        total_spot_fees = sum(a.spot_account.total_fees for a in self.agents)
        total_futures_fees = sum(a.futures_account.total_fees for a in self.agents)
