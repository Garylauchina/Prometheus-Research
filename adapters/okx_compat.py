...
def import_okx_module(module_name):
    ...
    # 方式3: 尝试直接从okx导入特定的API类（适用于python-okx）
    try:
        # 为不同模块定义对应的API类名称
        api_class_map = {
    ...
    for module_name in required_modules:
        ...
        if module is not None:
            ...
            if hasattr(module, api_class_name):
                ...
                logger.warning(f"{module_name}模块中缺少{api_class_name}类，将创建模拟类")
                ...
                if module_name == 'MarketData':
                    class MarketDataAPI:
                        def __init__(self, api_key=None, api_secret_key=None, passphrase=None, flag=1):
                            self.flag = flag
                            logger.info(f"创建MarketDataAPI模拟类")
