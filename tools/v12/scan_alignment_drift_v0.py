...
def _default_field_mappings() -> List[Mapping]:
    ...
    return [
        ...
        # order type naming drift (observed in broker evidence): ord_type vs order_type
        Mapping("ord_type", "ordType", "snake_case_to_camel"),
        Mapping("order_type", "ordType", "snake_case_to_camel"),
        Mapping("side", "side", "identity"),
...
def _filter_observed_keys(raw: Set[str]) -> Set[str]:
    ...
    drop_exact = {
        ...
        "agent_id_hash",
        "decision_ref",
        # broker gate / execution metadata (not order params)
        "gate_name",
        "gate_decision",
        "gate_reason_code",
        "gate",
