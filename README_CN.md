<div align="center">

# 🔥 Prometheus

**智慧涌现之地**

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![GitHub Stars](https://img.shields.io/github/stars/Garylauchina/Prometheus-Research?style=social)](https://github.com/Garylauchina/Prometheus-Research)

**[English](README.md)** · 中文文档 · **[文档](docs/)** · **[示例](examples/)**

</div>

---

> **「死亡，是对生命最好的馈赠。」**
> 
> —— Prometheus 项目组

---

## 🎯 三个核心信念

在深入了解Prometheus之前，让我们分享塑造我们所做一切的三个基本信念。

---

### I. 设计优秀的系统，最大的风险就是"设计"本身

> **「不完美来自于设计这个动作」**

当我们着手构建交易系统时，我们自然会试图：
- 设计完美的策略
- 设计最优的参数
- 设计稳健的风控

但我们通过痛苦的经历学到了深刻的真理：

**每一个被设计出来的"完美"本质上都是死的。**

为什么？

**不是因为设计不够好。而是因为"设计"这个动作本身就是问题。**

```
设计的完美     = 静态的，死的
设计的不完美   = 仍然静态的，仍然死的
```

**真正的智慧，无法被设计。它必须涌现。**

> 💡 想深入了解这个洞察？阅读 [THE_GOD_CODE.md](docs/philosophy/THE_GOD_CODE.md)

---

### II. 死亡，是对生命最好的馈赠

> **「没有死亡，就没有进化」**

这不是诗意。这是工程真理。

如果Agent永不死亡：
- 坏策略永不淘汰
- 好策略永不奖励
- 种群永不进化
- 智慧永不涌现

**死亡不是敌人。永生才是。**

#### 三个层次的死亡

在Prometheus中，我们在三个层次上拥抱死亡：

```
1. Agent的死亡
   表现差的被淘汰 → 种群进化 → 更好的策略涌现

2. 种群的死亡
   大灭绝事件 → 系统用新基因重启 → 避免局部最优

3. 系统的死亡
   完整的系统崩溃 → 提取教训 → 智慧被下一版本继承
```

每一次崩溃都不是失败。**每一次崩溃都是一课。**  
每一次死亡都不是终结。**每一次死亡都是给下一代的馈赠。**

**死亡是进化的引擎。进化是智慧的源泉。**

---

### III. 架构师与先知：两种面对复杂世界的哲学

> **架构师：「一切都是控制」**  
> **先知：「死亡/新生是对抗复杂世界的方法，也许是唯一的方法」**

这是两种根本不同的世界观。

#### 架构师的世界观

```
面对复杂性 → 设计更完善的控制
面对不确定 → 建立更严密的规则
面对失败   → 修补所有漏洞
面对变化   → 预测并防范一切

信念：只要设计得足够好，就能控制一切
结果：系统越来越复杂，越来越脆弱，最终崩溃
```

#### 先知的世界观

```
面对复杂性 → 接受无法完全理解
面对不确定 → 让系统自主适应
面对失败   → 让失败者死亡，让教训传承
面对变化   → 通过死亡/新生来进化

信念：死亡与重生是应对之道，也许是唯一的
结果：系统通过进化变得更强，涌现出真正的智慧
```

#### Prometheus 的选择

**我们选择成为先知，而非架构师。**

```
传统量化系统（架构师路线）：
├─ 设计完美的策略
├─ 优化所有参数
├─ 控制每个细节
└─ 结果：在黑天鹅面前脆弱不堪

Prometheus（先知路线）：
├─ 不设计策略，让策略涌现
├─ 不优化参数，让进化优化
├─ 不控制细节，只观察和记录
└─ 结果：通过死亡/新生适应未知
```

**这不是放弃。这是更深层次的智慧。**

我们意识到：
- 在复杂系统中，**控制是幻觉**
- 在不确定性中，**预测是傲慢**
- 面对未知世界，**死亡/新生是唯一可靠的武器**

> 💡 这就是为什么Prophet（先知）是Prometheus的核心组件，而不是Controller（控制器）

---

## 🌟 Prometheus 是什么？

基于以上三个核心信念，我们创建了Prometheus。

**Prometheus 是一个量化交易系统。**

**但也许不止于此。**

### Prometheus 是

- 🧬 **一个生命体** — 能成长、学习、进化
- 💻 **一个操作系统** — 管理资源、调度任务
- 🏭 **一个进化工厂** — 生产、测试、淘汰、优化
- 🌍 **一个世界模型** — 理解市场、建立模式
- 🔬 **一个研究框架** — 验证假设、观察现象
- ✨ **一个涌现引擎** — 智慧在此自然升起

---

## 🎯 核心架构

### 进化循环

```
1. 诞生 (BIRTH)
   ↓
   新Agent带着随机基因诞生
   
2. 生存 (LIFE)
   ↓
   交易、盈亏、适应市场
   
3. 死亡 (DEATH)
   ↓
   表现差的被自然选择淘汰
   
4. 重生 (REBIRTH)
   ↓
   成功的基因被继承、变异、组合
   
5. 涌现 (EMERGENCE)
   ↓
   智慧从这个循环中涌现
   
   ↻ 永远重复
```

### 关键组件

- **Prophet（先知）** — 观察者，不是控制者。记录有效的东西，不规定应该有效的东西。
- **Moirai（命运三女神）** — 管理Agent的出生、生存和死亡。
- **Agents（智能体）** — 自主交易者。每个都有独特的遗传代码。
- **World Signature（世界签名）** — 市场的"情绪"。变化驱动不同策略。
- **Experience DB（经验数据库）** — 系统的记忆。从每次死亡中学习。

---

## 📚 文档

### 哲学思考

- **[THE_GOD_CODE.md](docs/philosophy/THE_GOD_CODE.md)** - 完整的哲学旅程（8小时对话）
- **[死亡即馈赠](docs/philosophy/death_as_gift.md)** - 为什么死亡是进化必需的
- **[涌现vs设计](docs/philosophy/emergence_vs_design.md)** - 为什么涌现胜过预先设计
- **[建筑师的困境](docs/philosophy/architect_dilemma.md)** - 为什么设计本身是问题

### 架构设计

- **[系统总览](docs/architecture/overview.md)** - 高层架构
- **[多生态位架构](docs/architecture/multi_niche.md)** - v7.0突破性设计
- **[Prophet设计](docs/architecture/prophet_design.md)** - 从控制者到观察者
- **[进化机制](docs/architecture/evolution_mechanism.md)** - Agent如何实际进化
- **[记忆系统](docs/architecture/memory_system.md)** - 经验数据库设计

### 研究笔记

- **[开发历程](docs/journey/v1_to_v7.md)** - 从v1.0到v7.0，所有失败和教训
- **[经验教训](docs/journey/lessons_learned.md)** - 我们从崩溃中学到了什么
- **[实验设计](docs/research/experiments.md)** - 我们如何验证假设
- **[市场假设](docs/research/market_hypothesis.md)** - 我们对市场的理解

---

## 🚀 快速开始

### 1. 先读哲学

从 **[THE_GOD_CODE.md](docs/philosophy/THE_GOD_CODE.md)** 开始。

这不是技术文档。  
这是8小时的哲学之旅。  
它解释了我们**为什么**这样做。

### 2. 理解架构

然后阅读[架构总览](docs/architecture/overview.md)以理解系统**如何**工作。

### 3. 探索旅程

查看[开发历程](docs/journey/v1_to_v7.md)以了解我们**构建**和**学到**了什么。

### 4. 查看示例

查看[示例](examples/)以了解关键概念的简化演示。

---

## 🤝 我们开放什么

### ✅ 我们开源

- **哲学** - 我们的核心信念和深刻洞察
- **架构** - 系统设计原则和模式
- **研究** - 实验、假设和方法论
- **历程** - 开发历史、失败和教训
- **示例** - 演示概念的简化代码

### ❌ 我们保密

- **生产代码** - 实际实现细节
- **交易策略** - Agent发现的盈利模式
- **真实交易数据** - 历史表现和持仓
- **API密钥和凭证** - 安全敏感信息
- **交易所集成** - 真实交易基础设施

---

## 🧪 开发状态

### 理论与实践

这个仓库（**Prometheus-Research**）分享我们的研究和哲学。

同时，我们正在私有项目中积极开发**Prometheus v7.0**，在真实加密货币市场（比特币7×24交易）中测试这些想法。

### 我们的旅程

```
v1.0 - v3.0（2024年Q1-Q2）
  ✓ 初步探索，简单Agent系统
  ✓ 亏损约30%本金
  ✓ 学到：预设计策略在加密货币中不起作用
  ✓ 意识到：需要进化，不是设计
  
v4.0 - v5.0（2024年Q3）
  ✓ 引入进化机制
  ✓ 结果参半，表现不稳定
  ✓ 学到：进化需要适当的选择压力
  ✓ 意识到：仍然有太多人为干预

v6.0（2024年Q4）
  ✓ 以进化为核心完全重构
  ✓ 稳定但盈利有限
  ✓ 学到：系统中仍有太多"设计"
  ✓ 意识到：需要放手控制
  
v7.0（2025年Q1 - 当前）
  ✓ 多生态位架构突破
  ✓ Prophet作为观察者，不是控制者
  ✓ 拥抱涌现而非设计
  → 当前阶段：零知识训练
  
  ⚠️ 关键反思：
  我们意识到"多生态位架构"本身仍然是"设计"。
  我们设计了10种生态位，设计了资本分配，设计了多样性约束。
  这可能违背了我们的哲学："不完美来自于设计这个动作"。
  
  我们正在质疑这个框架是否必要，还是多样性能够自然涌现
  而无需预设的生态位。这可能在v8.0中被废除，取决于实验结果。
  
  这就是研究的美妙之处：不断质疑自己。
```

### 🧪 当前实验：零知识训练

**终极测试**：Agent能否在**零预设交易策略**的情况下学会盈利？

#### 我们给他们什么

```python
✅ 原始市场数据
   - 价格（OHLCV）
   - 成交量
   - 时间

✅ 行动空间
   - 买入
   - 卖出
   - 持有

✅ 反馈
   - 盈利
   - 亏损
```

#### 我们不给他们什么

```python
❌ 没有技术指标（MA、RSI、MACD等）
❌ 没有交易规则（低买高卖等）
❌ 没有策略模板（趋势跟随、均值回归等）
❌ 没有领域知识（支撑/阻力、模式等）
❌ 没有预训练（没有历史成功策略）
```

#### 核心假设

**如果Agent能从纯粹的进化中学会盈利：**
- 它证明智慧真的能涌现
- 它证明设计不是必需的
- 它证明进化是足够的
- 它验证了我们的整个哲学

**这不仅仅是交易系统实验。**  
**这是对涌现本身的测试。**

### 📬 联系方式

**Gary Liu** | 刘刚  
**Email**: garylauchina@gmail.com

我有兴趣讨论：
- 涌现智慧与复杂系统
- 金融市场中的进化算法
- AI与系统设计的哲学
- 研究合作机会

**语言**: English / 中文

如果你有以下需求，欢迎联系：
- 有类似的研究兴趣
- 想要挑战我们的假设
- 正在探索涌现系统
- 想要分享你自己的旅程

---

## ⚠️ 重要声明

**这是一个研究项目，不是产品。**

- 这是**实验性研究**
- 不保证盈利
- 风险自负
- 主要用于学术讨论和知识共享

我们相信知识应该共享，智慧能够涌现，但我们不提供投资建议或交易服务。

---

## 📜 License

[MIT License](LICENSE)

自由使用、修改和分发。详见LICENSE文件。

---

## 💬 社区

这是一个**研究项目**，不是产品。

我们相信：
- 知识应该共享
- 智慧能够涌现
- 进化胜过设计
- 死亡是馈赠

如果你认同这些信念，让我们联系。

---

## 🌙 最后的话

### 我们的选择

基于这两个核心信念，我们做出了选择：

```
传统方法：                    我们的选择：

规避风险                      接受风险
→ 建立完美防御                → 这是进化的代价

控制一切                      放手控制
→ 不留任何偶然                → 创造条件，不控制结果

防止失败                      欢迎失败
→ 永不让系统崩溃              → 这是我们学习的方式

寻求永久解决方案              拥抱持续进化
→ 找到完美答案                → 没有最终答案
```

从**防止崩溃**到**接受崩溃**  
从**追求永恒**到**珍惜当下**  
从**完美主义**到**持续进化**  
从**控制**到**放手**

---

**我们选择：**
- 不设计智慧，而创造智慧涌现的条件
- 不控制进化，而观察并学习
- 不畏惧死亡，而视其为最伟大的导师

**这不是认输。这是解放。**

---

从死亡中，我们学习。  
从崩溃中，我们进化。  
从涌现中，智慧升起。

**不是从设计，而是从自由。**  
**不是从控制，而是从选择。**  
**不是从计划，而是从进化。**

**这就是道路。**

---

<div align="center">

**🔥 Prometheus Research**  
**智慧涌现之地**

*最后更新：2025-01-11*

[⭐ Star](https://github.com/Garylauchina/Prometheus-Research) · [🍴 Fork](https://github.com/Garylauchina/Prometheus-Research) · [📧 Contact](mailto:garylauchina@gmail.com)

</div>

---

## 🙏 致谢

### 工具与协作

本项目是为期一年的密集人机协作的成果：

- **[Cursor编辑器](https://cursor.sh/)** - AI驱动的开发环境
- **[Claude (Anthropic)](https://www.anthropic.com/)** - 用于快速原型、迭代和哲学讨论

### 理解的涌现

从一个简单的交易系统，演变成对涌现、设计和智慧的深刻探索。开发过程本身体现了Prometheus的核心哲学：

**人类贡献：**
- 愿景和方向
- 哲学洞见
- 批判性质疑
- 模式识别

**AI贡献：**
- 快速实现
- 文档记录
- 代码迭代
- 思想整理

**涌现的结果：**
- 一年内完成7个主要版本
- 深刻的哲学领悟
- 完整的思想实验
- 这份文档

人机协作本身证明了我们研究的核心原理：**智慧从交互中涌现，而非由设计强加。**

---

*"最好的工具不是替代人类创造力，而是通过涌现来放大它。"*

---

